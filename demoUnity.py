...
# Función para extraer los parametros de SMPL-X
def extract_smplx_params(humans):
    smplx_params_list = []
    for human in humans:
        smplx_params = {
            "pose": human['pose'].tolist(),
            "shape": human['shape'].tolist(),
            "expression": human['expression'].tolist(),
            "location": human['loc'].tolist(),
            "distance": human['transl'][2].item(),
            "rotation": human['pose'][0].tolist(),
            "positionOffset": [0, 0, 0],
            "rotationOffset": [0, 0, 0]
        }
        smplx_params_list.append(smplx_params)
    return smplx_params_list
...
if __name__ == "__main__":
        ...
        parser.add_argument("--save_json", type=int, default=1, choices=[0,1], help='save SMPL-X parameters as JSON')
        ...
        for i, img_path in enumerate(tqdm(l_img_path)):
            ...
            if args.save_json:
                smplx_params = extract_smplx_params(humans)
                json_filename = os.path.join(args.out_folder, f"{Path(img_path).stem}_{model_name}_smplx_params.json")
